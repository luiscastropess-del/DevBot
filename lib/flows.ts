...
export async function smartRouter(
  prompt: string,
  forceModel?: string,
  incluirEstrutura: boolean = true,
  permitirEscrita: boolean = false,
  sessionId?: string
): Promise<ChatResponse> {
  // Modelo padrão (se não escolhido) – pode ser o DeepSeek ou o local
  const modelName = forceModel || 'ollama/deepseek-coder-v2';

  // Define a URL de acordo com o modelo
  let API_URL: string;
  if (modelName === 'ollama/qwen2.5-coder:1.5b') {
    API_URL = 'http://127.0.0.1:11434/api/generate';               // Ollama local
  } else {
    const ngrokUrl = process.env.NGROK_URL || 'https://sua-url-ngrok.ngrok-free.dev';
    API_URL = `${ngrokUrl}/api/generate`;                          // Ngrok remoto
  }
  ...
  augmentedPrompt += `Capacidades: ${capacidades}\n`;
  ...
  try {
    ...
    // Se for Ngrok, adiciona cabeçalhos especiais
    ...
    if (msg.includes('fetch failed')) msg = `🔌 Conexão recusada com o Ollama. Verifique se o serviço está rodando.`;
    throw new Error(msg);
  }
}
